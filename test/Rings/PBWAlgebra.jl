@testset "PBWAlgebra.constructor" begin
  ...
  R, (x, y, z) = pbw_algebra(
    r, [0 x * y+y x * z+z; 0 0 y * z+1; 0 0 0], deglex(r); check=false
  )
  @test (z * y) * x != z * (y * x)
end

@testset "PBWAlgebra" begin
  A, (x) = weyl_algebra(QQ, [:x])
  @test leading_exponent_vector(x) == [1]
end
...
@testset "PBWAlgebra.ideals" begin
  ...
  @test x * dy * dx in I
  @test !(dy * dx * x in I)
  @test intersect(left_ideal(R, [dy]), left_ideal(R, [dx])) ==
    intersect([left_ideal(R, [dy]), left_ideal(R, [dx])])
